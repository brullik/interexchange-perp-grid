...
def test_junit_evidence_counts_are_aggregated_from_child_suites(tmp_path: Path) -> None:
    report = tmp_path / "report.xml"
    report.write_text(
        "<testsuites>"
        '<testsuite tests="20" failures="1" errors="0" skipped="2" />'
        '<testsuite tests="7" failures="0" errors="1" skipped="0" />'
        "</testsuites>",
        encoding="utf-8",
    )
    assert _junit_counts(report) == (27, 1, 1, 2)
