...
- **Current checkpoint:** protected `main` `459a3d6eef5f788066dc73cba2298918abbf9945` continuously enables all seven configured venues for capability-gated public observation while preserving immutable Wave 1 live authority; post-merge run `32462202532` passed all five jobs. Exact exposure-safe upgrade code head `7832e414faded4d7bcbd1f7c21e24e23becf62b4` passed run `32463480587` 5/5 with exact operations, replay, C4 critical, C4.3, and security artifacts. This STATUS-only evidence rebind requires its own fresh exact-head CI and final review. Remaining independent software work after this gate is complete owner-onboarding/canary handoff
...
| Exposure-safe immutable upgrade | EXACT_CODE_GATE_PASS / EVIDENCE_REBIND | `shadow-upgrade.sh` arms a separate durable SQLite entry freeze before stopping the service and refuses shutdown when any active journal action exists. Normal and standalone-emergency action creation plus FLAT reactivation check the same gate inside their `BEGIN IMMEDIATE` transaction; already-owned recovery/close transitions remain available. Backup failure restarts the old service before release; unhealthy upgrade restores the frozen SQLite backup and old immutable digest before release; exact healthy deployment identity is checked before release; release failure remains explicit and fail-closed. Twenty concurrent arm/create races, restart persistence, active-action refusal, recovery, rollback, and CLI wiring are executable tests. Exact code head `7832e414` passed run `32463480587` 5/5. Operations artifact `9439948567` proves exact deployment identity, healthy restart, real failed-upgrade rollback, 50-action recovery, 100 simulated reducing closes, stable FLAT, and zero production transports; replay `9439841034` passed 91 scenarios; C4 critical `9439809522` passed 30/30 with production submits 0; C4.3 `9439810797` passed 8/8 with false success/submits 0; security `9439801911` passed. Full local gate: lock64, Ruff113, mypy111, pytest 685 passed/9 platform-only skips, doctor shadow/live=false, diff-check PASS. This evidence-only rebind changes no code and is accepted only by fresh exact-head CI plus final P0/P1/P2=0 review |
