...
- **Current checkpoint:** `LOCAL_GATE_PASS / EXACT_GATE_REQUIRED` for the Windows S4U qualification runner. A LocalMachine DPAPI envelope is created only from the existing CurrentUser profile, remains outside Git, has inherited ACLs removed, and is readable only by the same Windows SID; the loader avoids PowerShell Security-module autoloading, preserves the fixed route, and forces `shadow` plus `live=false`. The noninteractive Task Scheduler session now survives Codex terminal cancellation, while dirty source still fails before qualification. Windows-equivalent `make verify` passed lock64, Ruff118, strict mypy118, pytest714/16 platform skips, and doctor `shadow/live_orders_allowed=false`. Exact commit, CI/artifacts, protected-main merge, 24-hour qualification, real orders, and the eight-hour post-FLAT interval remain pending; no live or canary authority is claimed.
