...
- **State:** PHASE5_4_MEXC_TEST_HARDENING_LOCAL_PASS_AWAITING_EXACT_GATE
- **Current checkpoint:** Phase 5.4 MEXC fail-closed code head `fcb0e45` passed independent technical review; the sole exact-run failure was an unrelated one-second scheduler-sensitive smoke-test wait, now hardened locally. A test-only checkpoint, exact-head CI/artifacts, and independent final review remain required. MEXC broad BBO, contract writes, and live remain disabled
...
gate remains lock64/Ruff101/mypy99/pytest585/doctor shadow-live=false. A test-only checkpoint,
fresh exact-head CI/artifacts, and independent final review remain pending; neither
`32421228611` nor `32422342149` is final MEXC evidence.
```
