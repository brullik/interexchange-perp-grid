...
| Native laptop pre-VPS gate | LOCAL_RUNTIME_PASS / EXACT_REBIND_REQUIRED | `scripts/laptop_preflight.py` runs the Windows-equivalent full verify plus real public read-only Wave 1 scan, Parquet/DuckDB replay, SQLite WAL, continuous shadow heartbeat, clean shutdown, and restart without Docker, private credentials, or live authority. Native Python 3.12.13 evidence: lock64/Ruff114/mypy114/pytest696+16 platform skips/doctor PASS; exact runtime produced 100 common instruments, 6 eligible routes, 2 eligible Wave 1 routes, qualified Bybit+OKX, 283 persisted Parquet levels, zero pending files, SQLite WAL, healthy heartbeat, and restart count 1→2. The bounded first-handshake remediation now needs an exact commit, full unchanged-head rerun, CI, and final review before this gate is COMPLETE or any VPS upload. |
...
```text
2026-08-21 native Windows laptop pre-VPS gate without Docker: LOCAL PASS
...
- pytest: 696 passed, 16 platform-only skips
...
- persistence: 283 Parquet levels, zero .pending files, SQLite WAL present
- native service: healthy twice; restart counter 1 -> 2; clean shutdown
- Docker: deliberately not required or invoked on the laptop

The first-qualified WebSocket progress fix remains uncommitted at this status update. A full exact-head
rerun, fresh CI/artifacts, and final review are required before the laptop gate is COMPLETE or any VPS
upload begins.
```
