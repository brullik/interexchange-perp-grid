...
- **State:** SOFTWARE_COMPLETE_CANDIDATE
- **Current checkpoint:** protected `main` `0ef5064fb1307afb02eae7e2f56c215035e7e97d` contains the independently reviewed exposure-safe immutable-upgrade remediation. Post-merge run `32469624842` passed all five jobs; exact artifacts are operations `9442161461`, replay `9442066176`, C4 critical `9442036362`, C4.3 `9442030809`, and security `9442019867`. The final software-only onboarding checkpoint removes owner technical route selection, fixes qualification to the locked `BTC:bybit>okx` default in both the wizard and runtime, requires factual dedicated-subaccount/permission/IP/funding confirmations, and makes the autonomous orchestrator fail closed without the resulting local receipt or on route drift. Windows-equivalent locked gate: lock64, Ruff113, mypy111, pytest 692 passed/16 platform-skipped, doctor shadow/live=false, diff-check. Exact branch CI, independent unchanged-head review, merge, and a current `v1.0.0-rc2` image remain required before declaring `SOFTWARE_COMPLETE_WAITING_OWNER_ONBOARDING`.
...
| Continuous seven-venue public runtime | EXACT_CODE_GATE_PASS / EVIDENCE_REBIND | Production shadow evaluation and `public-scan` instantiate all configured Wave 1/2/3 public venues. Capability and instrument failures isolate only the affected expansion venue; missing factories retry transactionally; cancellation-resistant probes have a hard deadline, cannot publish late qualification, coalesce to one owned task per venue, and make shutdown fail explicitly while nonterminal. P6/broad shedding defers periodic expansion probes while preserving Wave 1 and explicit force/reconnect. The capability matrix keeps Bitget/KuCoin public-only and BingX/MEXC quarantined when required capabilities are absent; live/canary authority remains immutable Wave 1 and `execution_authorized=false`. Exact code head `b3abede` passed run `32460803174` 5/5; operations artifact `9438995796`, replay `9438907327`, C4 critical `9438883979`, C4.3 `9438882897`, and security `9438871393` are exact-head-bound and nonexpired. Local gate: lock64, Ruff113, mypy111, pytest 681 passed/6 platform-skipped, doctor shadow/live=false; this STATUS-only rebind requires fresh exact-head CI and final review. |
