...
- **2026-08-25 laptop qualification containment:** the first post-merge qualification attempts were invalidated before an epoch could be accepted. Concurrent/interruptible launch paths caused Telegram polling conflicts, KuCoin teardown warnings, and third-party HTTP logging exposed the Telegram bot token in a local ignored log; the runner was stopped, live remained false, local token-shaped log content was redacted, and the credential now requires owner rotation. The corrective exact-code checkpoint disables Telegram only inside non-interactive laptop qualification and adds a process-wide raw/URL Telegram-token redaction filter while suppressing `httpx`/`httpcore` INFO request logging. An isolated 30-minute rehearsal policy uses unique per-run SQLite/market roots, 1,800 seconds, 500 synchronized observations per venue, three funding checkpoints and unchanged zero-error/gap/clock gates; stale smoke epochs cannot satisfy a new run, and the result is explicitly non-accepting and cannot authorize canary/live. A new 12-hour epoch must start from zero only after the rehearsal and exact locked gates pass.
