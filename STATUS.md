...
- **Owner-requested laptop gate:** native Windows verification is in progress before any VPS upload; Docker is explicitly excluded. The new gate found and locally remediated malformed batch isolation, bounded broad-universe capacity, CCXT future/session teardown, and hard shutdown defects. The remaining machine prerequisite is Windows Time synchronisation: the stopped `W32Time` service leaves exchange clock skew at approximately 1.8 seconds versus the locked 1-second maximum, so the exact production configuration correctly remains fail closed.
...
| Native laptop pre-VPS gate | IN_PROGRESS / OWNER_OS_ACTION | `scripts/laptop_preflight.py` runs the Windows-equivalent full verify plus real public read-only Wave 1 scan, Parquet/DuckDB replay, SQLite WAL, continuous shadow heartbeat, clean shutdown, and restart without Docker, private credentials, or live authority. A diagnostic scan with only a temporary non-evidence clock tolerance proved 100 bounded common instruments, 6 eligible routes, and zero unhandled futures/callbacks/sessions; the exact production gate still rejects all venues because Windows Time is stopped and measured skew is about 1.8 seconds. |
...
### Laptop Windows Time synchronisation

- **Exact required action:** open PowerShell as Administrator and run `Set-Service W32Time -StartupType Automatic`, `Start-Service W32Time`, and `w32tm /resync /force`; then run `.venv\Scripts\python.exe scripts\laptop_preflight.py` from the repository root.
- **Why Codex cannot do it:** the current process has no Windows Service Control Manager administrator token; the attempted service start was denied by the operating system.
- **Validation:** `w32tm /query /status` reports a running synchronised source and `scripts/laptop_preflight.py` writes a `status=PASS` report with at least two qualified Wave 1 venues, at least two eligible paired Wave 1 routes, replayable Parquet levels, healthy heartbeat, and a restart count increment.
- **Fail-closed behavior:** while measured venue clock skew exceeds 1000 ms, every affected public venue remains quarantined, no route is eligible, live remains disabled, and no order transport is reachable.
