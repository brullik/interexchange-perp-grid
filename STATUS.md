...
- **State:** PHASE3_1_EXACT_013F_P1_REMEDIATION_LOCAL_GATE
- **Current checkpoint:** Draft PR #5 exact head `013f1d5` passed all five GitHub jobs but failed independent lifecycle review on late initial probe/discovery mutation, repeated startup ownership, refresh/shutdown coordination, and partial factory rollback; one idempotent transactional lifecycle now owns startup, refresh, recovery, and bounded shutdown locally, with the complete Windows-equivalent gate green pending a new exact head
