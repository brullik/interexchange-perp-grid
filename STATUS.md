...
- **Current checkpoint:** protected `main` `459a3d6eef5f788066dc73cba2298918abbf9945` continuously enables all seven configured venues for capability-gated public observation while preserving immutable Wave 1 live authority; post-merge run `32462202532` passed all five jobs. The exposure-safe upgrade checkpoint now passes the full local locked gate: one durable SQLite gate atomically blocks every new normal or standalone-emergency live action, refuses shutdown while any active action exists, preserves existing recovery transitions, and releases only after exact new-image health or successful rollback. Exact checkpoint commit, CI/artifacts, and independent review remain required. Remaining independent software work after this gate is complete owner-onboarding/canary handoff
