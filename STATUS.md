...
- **Owner-authorized one-time 12-hour laptop exception:** LOCAL_GATE_PASS / EXACT_GATE_REQUIRED. The exception is a Windows-native, locally confirmed policy profile whose evidence records 43,200 seconds while every other locked gate remains identical. `config/defaults.yaml`, `config/RUNTIME_POLICY.yaml`, generic qualification and all future VPS qualification remain at 86,400 seconds with their standard five-second scan interval. Only the short laptop runtime uses a three-second shadow-cycle interval, leaving configured cadence headroom for the unchanged 10,000-synchronised-snapshot requirement; finalization still uses actual persisted counts and zero-error gates. Risk-stage promotion and production canary reject the short profile unless the exact local Windows receipt is present; the receipt is not a live-money consent, is removed after the workflow, and cannot change YAML/VPS policy. A completed short-policy canary is durably detected by qualification hash, so the same evidence cannot authorize a second canary after restart. Windows-equivalent `make verify` passed lock64, Ruff118, strict mypy118, pytest 719 passed/16 platform skips, and doctor `shadow/live_orders_allowed=false`; Bandit medium/high is zero. Exact commit, CI, independent review, the actual 12-hour run, a separate short-lived live-money confirmation, real fills and eight-hour post-trade evidence remain pending; no real order was submitted.
