...
- **State:** SOFTWARE_COMPLETE / EXTERNAL_QUALIFICATION_PENDING
- **Current checkpoint:** code head `bd24c6301ea37ca52d6e079662d014abe55082c4` completes the Windows S4U qualification runner. A LocalMachine DPAPI envelope is created only from the existing CurrentUser profile, remains outside Git, has inherited ACLs removed before ciphertext is written, and is readable only by the same Windows SID; the loader avoids PowerShell Security-module autoloading, preserves the fixed route, and forces `shadow` plus `live=false`. The noninteractive Task Scheduler session survives Codex terminal cancellation, while dirty source fails before qualification. Windows-equivalent `make verify` passed lock64, Ruff118, strict mypy118, pytest714/16 platform skips, and doctor `shadow/live_orders_allowed=false`. Exact run `32742047320` passed 5/5; operations `9525694271`, replay `9525604780`, C4 critical `9525562411`, C4.3 `9525558659`, and security `9525551700` are exact-head-bound and nonexpired. This STATUS-only evidence rebind requires fresh exact-head CI before Ready. Protected-main merge, 24-hour qualification, real orders, and the eight-hour post-FLAT interval remain pending; no live or canary authority is claimed.
