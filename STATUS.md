...
- **Current checkpoint:** protected `main` `459a3d6eef5f788066dc73cba2298918abbf9945` continuously enables all seven configured venues for capability-gated public observation while preserving immutable Wave 1 live authority; post-merge run `32462202532` passed all five jobs. Exposure-safe upgrade code checkpoint `5d5674b6c1310bc4483c8dce85b2d31ffc6540cb` takes an exact old-image backup even when the service is stopped, acquires a host single-flight lock, and arms a release-SHA-owned legacy account-flatten lease before any target migration. Active exposure therefore restarts/resumes the exact old recovery image without schema mutation; owner pause state is never overwritten. Zero-exposure upgrade/rollback uses the verified target image for a failure-atomic SQLite main/WAL/SHM restore, retains rollback files until install plus directory fsync succeeds, and releases only its exact gate after healthy immutable identity. Exact run `32468213977` passed all five jobs; exact non-expired artifacts are operations `9441655484`, replay `9441542487`, C4 critical `9441510922`, C4.3 `9441509836`, and security `9441500580`. An evidence-only STATUS rebind, its fresh exact-head CI, and final unchanged-head review remain required. Remaining independent software work after this gate is complete owner-onboarding/canary handoff
