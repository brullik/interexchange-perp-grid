...
- **A4/A5/A6 integration boundary:** the common non-executable core derives entry/exit VWAP from exact raw L2, measures book impact, scales reserves after lot rounding, derives reverse targets from actual entry, enforces reference plus three fresh synchronized confirmations, exact model/grid/runtime identity, two-venue funding, actual-fee inputs, locked economics and route/portfolio sizing, and emits one immutable intent for replay, shadow and the existing protected live coordinator boundary. Public Wave 1 snapshots expose immutable raw books and quality/funding identity; the shared shadow bridge persists simulated paired fills, exits on the same target/stop priority, rearms only after stable-FLAT retreat, and uses durable decision cycles across restart. `scripts/laptop-aggressive.ps1` adds no-Docker verify/shadow/qualify/canary/pilot/status/stop orchestration; public shadow refreshes closed 1m history each minute and cannot submit. Exact A5 checkpoint `f4dce73` passed CI run `32796846331` 5/5. Aggressive qualification writes a separately hash-protected, non-executable binding only when accepted route qualification, native runtime, commit/source/config, model/profile/reference/source manifests and both exact five-level geometries agree; stale or tampered binding blocks pilot. Abrupt spawned-process tests preserve every active grid transition, and the qualification progress subprocess now uses a lean read-only worker so the production service deadline remains bounded. The expanded Windows-equivalent gate passed lock64, Ruff131, strict mypy70 source modules, pytest785 passed/16 platform skips and doctor `shadow/live_orders_allowed=false`. A7 exact review remains next; canary still requires separate live-money authorization and production submit is zero.
