...
- **State:** AGGRESSIVE_SYMBIOSIS_V1 / A4_CORE_LOCAL_PASS / A4_A5_WIRING_NEXT / LIVE_DISABLED
...
- **A4 integration boundary:** the common non-executable core now enforces reference+three fresh synchronized L2 confirmations, full VWAP depth, explicit state/history/regime gates, two-venue funding schedules, four private taker fees, all locked reserves, 1.35/0.15 economics, 4.50/5.00 and 45/50 loss caps, local-margin sizing, deterministic route scoring and the shared exit-priority function. Its tree passed lock64, Ruff128, mypy128, pytest771 passed/16 skips, Bandit medium/high zero and doctor live=false. Automatic post-fill recomputation and wiring the shared exit function into replay/shadow/live remain intentionally unchecked and are the next work; no strategy intent reaches a submit path yet.
