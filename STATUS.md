...
- **State:** PHASE5_1_BITGET_CLASSIC_AWAITING_EXACT_GATE
- **Current checkpoint:** Phase 5.1 Bitget Classic public/read-only/private contract code passed the local locked gate and independent dirty-tree P0/P1/P2=0 review; checkpoint commit, push, exact-head CI/artifacts, and exact-head review remain pending. Bitget live remains disabled and is not part of the Wave 1 canary allowlist
...
| Phase 5.1 Bitget Classic code candidate | LOCAL_TECHNICAL_PASS | Bitget is now a typed venue profile with a Classic-only CCXT Pro transport, exact 4096-byte batch ticker framing and matching unsubscribe acknowledgement, raw books15 sequence propagation/regression rejection, exact linear-USDT discovery, split account-wide read-only snapshot/stream params, and final pinned protected IOC `clientOid`/crossed/force mapping. Wave 1 remains exactly Binance USD-M/Bybit/OKX; Bitget is explicitly denied at the live-canary submit boundary. Local locked gate: lock64, Ruff96, mypy94, pytest547, doctor shadow/live=false, Bandit medium/high0, diff-check; independent dirty-tree review P0=0/P1=0/P2=0. No credentials or network evidence were fabricated; commit and exact-head CI/review remain pending. |
| C6 venue expansion | IN_PROGRESS | Bitget Classic Phase 5.1 code candidate is locally green; KuCoin Classic, BingX, MEXC, seven-venue runtime matrix, FT-02 isolation, and final operations gates remain pending. |
