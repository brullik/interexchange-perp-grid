...
- **Current checkpoint:** protected `main` `9d99e5bb774386de9314ad2cab125096675442a1` contains the complete software-only product and passed post-merge run `32473019160` in all five jobs. Exact artifacts are operations `9443378332` (42/42 acceptance criteria, zero production transports), replay `9443314750`, C4 critical `9443285592`, C4.3 `9443280252`, and security `9443267601`. Independent final onboarding review reported P0=0/P1=0/P2=0. Annotated prerelease [`v1.0.0-rc2`](https://github.com/brullik/interexchange-perp-grid/releases/tag/v1.0.0-rc2) targets this exact main; publisher run `32473449873` and manifest artifact `9443436103` bind both GHCR tags to `sha256:2c49fd425cdc9d247d9cc729e83ecb0f53e3c735d7e815ab279cee140bfe26da`. No VPS, credentials, real 24-hour qualification, canary, pilot, production operation, or profitability is claimed. Before the already-recorded external `ONE_TIME_ONBOARDING`, the owner requested the additional native laptop gate below; live and canary remain fail closed.
- **Owner-requested laptop gate:** native Windows verification passed before any VPS upload; Docker was explicitly excluded. Windows Time is `Running / Automatic`, three independent NTP samples were within -7 to +21 ms against the locked 1000 ms maximum, and exact code checkpoint `52e26651551f2f8724690a5cf97474455aa7bc9e` qualified the default Bybit/OKX pair. The gate found and remediated malformed batch isolation, bounded broad-universe capacity, CCXT future/session teardown, hard shutdown defects, and the conflation of first/reconnect WebSocket handshake latency with steady-state quote freshness. Exact code-head CI `32518816527` passed 5/5 with five SHA-bound artifacts. This evidence-only status rebind requires its own fresh exact-head CI/final review before Ready or merge; no VPS upload has begun.
- **Owner-selected native laptop live path:** a local checkpoint candidate now binds a clean CPython 3.12 interpreter, installed dependency set, lock, source, config, platform and exact release into one immutable native runtime manifest instead of pretending that Docker runs on the laptop. Windows DPAPI onboarding keeps restricted Wave 1 credentials outside Git and permanently excludes the local unlock/live consent; the automated workflow keeps W32Time and sleep prevention active, runs the locked 24-hour/10,000-snapshot qualification, permits only one separately confirmed minimum-notional paired canary, and accepts the final report only after four durable filled open/close legs, stable FLAT, zero active action and at least 28,800 post-trade service seconds. Local full gate passed with 708 tests and 16 platform-only skips. This is software evidence only: exact commit/CI/review, credentials, qualification, real orders and eight-hour runtime evidence remain outstanding, and live remains disabled.
...
| Native laptop pre-VPS gate | COMPLETE_CODE / EVIDENCE_REBIND_REQUIRED | `scripts/laptop_preflight.py` runs the Windows-equivalent full verify plus real public read-only Wave 1 scan, Parquet/DuckDB replay, SQLite WAL, continuous shadow heartbeat, clean shutdown, and restart without Docker, private credentials, or live authority. Exact code checkpoint `52e26651551f2f8724690a5cf97474455aa7bc9e`: Python 3.12.13, lock64/Ruff114/mypy114/pytest698+16 platform skips/doctor PASS; exact runtime report `20260821T192839Z` produced 100 common instruments, 6 eligible routes, 2 eligible Wave 1 routes, qualified Bybit+OKX, 278 persisted Parquet levels, zero pending files, SQLite WAL, healthy heartbeat, and restart count 1→2. Run `32518816527` passed 5/5; exact artifacts are operations `9459909568`, replay `9459857597`, C4 critical `9459806616`, C4.3 `9459805499`, and security `9459790813`. This status-only rebind needs fresh exact-head CI/final review before Ready/merge; VPS upload remains unopened. |
