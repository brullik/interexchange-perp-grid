...
- **Current checkpoint:** protected `main` `a654777841e34e2cad08c98b64c781f0bf8568b4` contains the complete software-only product and hardened external qualification probes. PR #18 exact run `32731987988` and protected-main run `32732500521` passed all five jobs. Only the locked Bybit/OKX pair is mandatory, Binance USD-M remains the first alternate, Binance discovery is scoped to linear futures, probe failures are sanitized, and failed private transports close without leaked sessions. The next checkpoint isolates Windows replay tests in a runtime-local temp directory and makes the required private probes perform authenticated read-only account access before qualification. No external qualification, real order, eight-hour post-FLAT interval, VPS deployment, or profitability evidence is claimed; live and canary remain fail closed.
