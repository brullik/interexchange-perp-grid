...
- **Current checkpoint:** protected `main` `c50c60f6a70dcc068eca7b3a857dbb9b5daefd5e` contains the complete software-only product and the corrected native Windows DPAPI onboarding dialog. PR #17 exact run `32729732452` passed all five jobs with five exact-SHA artifacts. The current checkpoint hardens the external qualification preflight: only the locked Bybit/OKX pair is mandatory, Binance USD-M remains the first alternate, Binance discovery is scoped to linear futures, probe failures are sanitized, and failed private transports close without leaked sessions. No external qualification, real order, eight-hour post-FLAT interval, VPS deployment, or profitability evidence is claimed; live and canary remain fail closed.
