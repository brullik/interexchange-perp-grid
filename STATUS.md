...
- **State:** AGGRESSIVE_SYMBIOSIS_V1 / A4_SHARED_INTENT_LOCAL_PASS / A5_RUNTIME_WIRING_NEXT / LIVE_DISABLED
...
- **A4/A5 integration boundary:** the common non-executable core now derives reverse targets from actual L2/VWAP entry, enforces reference plus three fresh synchronized confirmations, exact model/grid/runtime identity, two-venue funding, actual-fee inputs, locked economics and post-rounding route/portfolio sizing, and emits one immutable intent for replay, shadow and the existing protected live coordinator boundary. Actual-fill risk is recomputed against the same 4.50/5.00 and 45/50 limits; ambiguous replay minutes choose stop before target; accepted intents reserve exactly one persistent grid level and still cannot authorize or submit execution. Windows-equivalent `make verify` passed lock64, Ruff130, strict mypy130, pytest777 passed/16 platform skips and doctor `shadow/live_orders_allowed=false`. Continuous live-public shadow/history wiring and executable exit propagation through the running supervisor remain the next work; production submit is zero.
