...
- **State:** AGGRESSIVE_SYMBIOSIS_V1 / A5_COMPLETE / A6_CANARY_SOFTWARE_LOCAL_PASS / PILOT_A_PENDING / LIVE_DISABLED
...
- **Latest local checkpoint:** the shared LIVE-mode aggressive evaluator now emits a short-lived, hash-protected, non-authorizing level-1 intent which the existing canary runtime revalidates against exact qualification/model/profile/runtime identities before translating it into protected IOC journal ownership. The Windows wrapper keeps credentials in DPAPI, requires the separate owner phrase, local unlock and Telegram challenge, restores `shadow/live=false` on every exit, and builds canary stage evidence from completed journal cycles; intent generation and stage reporting cannot submit. Windows-equivalent verification passed lock64, Ruff134, strict mypy72, pytest793 passed/16 platform skips and doctor `shadow/live_orders_allowed=false`. The five-level `pilot_a` operator flow, exact CI and independent review remain pending; no real order or live consent is claimed.
