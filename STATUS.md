...
- **State:** AGGRESSIVE_SYMBIOSIS_V1 / A5_PUBLIC_SHADOW_LOCAL_PASS / A6_QUALIFICATION_BINDING_NEXT / LIVE_DISABLED
...
- **A4/A5/A6 integration boundary:** the common non-executable core derives entry/exit VWAP from exact raw L2, measures book impact, scales reserves after lot rounding, derives reverse targets from actual entry, enforces reference plus three fresh synchronized confirmations, exact model/grid/runtime identity, two-venue funding, actual-fee inputs, locked economics and route/portfolio sizing, and emits one immutable intent for replay, shadow and the existing protected live coordinator boundary. Public Wave 1 snapshots now expose immutable raw books and quality/funding identity; the shared shadow bridge persists simulated paired fills, exits on the same target/stop priority, rearms only after stable-FLAT retreat, and uses durable decision cycles across restart. `scripts/laptop-aggressive.ps1` adds no-Docker verify/shadow/qualify/canary/pilot/status/stop orchestration; public shadow refreshes closed 1m history each minute and cannot submit, while canary remains explicitly blocked without separate live-money authorization. Windows-equivalent full gate passed lock64, Ruff128, strict mypy68 source modules, pytest782 passed/16 platform skips and doctor `shadow/live_orders_allowed=false`. Aggressive qualification/acceptance hash binding and active-state process-kill evidence remain next; production submit is zero.
