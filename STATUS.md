...
- **State:** PHASE3_1_EXACT_7CCC_P1_REMEDIATION_LOCAL_GATE
- **Current checkpoint:** Draft PR #5 exact head `7ccc338` passed all five GitHub jobs but failed independent adversarial review because an uninitialised engine could create transports after a successful shutdown; every startup entry point is now guarded locally by the closed barrier, initialisation is serialized with bounded shutdown, and the complete Windows-equivalent gate is green pending a new exact head
