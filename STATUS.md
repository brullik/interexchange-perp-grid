...
- **State:** AGGRESSIVE_SYMBIOSIS_V1 / A1_LOCAL_PASS / A2_NEXT / LIVE_DISABLED
- **Current checkpoint:** actual `origin/main` is `8ef3ad3dbf746917a5fa6cb46f366634ea5747f9`; the owner-supplied Aggressive Symbiosis V1 package verified 8/8 SHA-256 entries and is installed in Draft PR #22 on the single branch `codex/aggressive-symbiosis-v1`. The prior adaptive-only laptop qualification was explicitly stopped and is not accepted for this strategy. Unchanged-code Windows baseline passed lock64, Ruff118, strict mypy118, pytest719/16 platform skips, and doctor `shadow/live_orders_allowed=false`; checkpoint `6764c88` then passed lock64, Ruff120, strict mypy120, pytest727/16 skips and the same doctor gate. A1 now has typed closed source-minute and canonical reference-spread bars, lexical pair plus two directed-route identities, deterministic Decimal log-ratio OHLC, explicit quality/version rejection, no forward-fill, incomplete-only higher aggregation, a minimal CCXT 1m-only capability, atomic idempotent Parquet partitions, DuckDB restart replay, deterministic manifest hashing, and a public `reference-history-proof` CLI with coverage/quality/hash evidence and `production_submit_calls=0`. The completed A1 tree passed focused tests 59/59 and the full locked gate: lock64, Ruff122, strict mypy122, pytest732 passed/16 platform skips, doctor `shadow/live_orders_allowed=false`. No live authority, VPS action, or new qualification is claimed.
