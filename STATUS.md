...
- **State:** AGGRESSIVE_SYMBIOSIS_V1 / A1_IN_PROGRESS / LIVE_DISABLED
- **Current checkpoint:** actual `origin/main` is `8ef3ad3dbf746917a5fa6cb46f366634ea5747f9`; the owner-supplied Aggressive Symbiosis V1 package verified 8/8 SHA-256 entries and is installed on the single branch `codex/aggressive-symbiosis-v1`. The prior adaptive-only laptop qualification was explicitly stopped and is not accepted for this strategy. Unchanged-code Windows baseline passed lock64, Ruff118, strict mypy118, pytest719/16 platform skips, and doctor `shadow/live_orders_allowed=false`. A1 now has typed closed source-minute and canonical reference-spread bars, lexical venue ordering, deterministic Decimal log-ratio OHLC, explicit quality rejection, no forward-fill, incomplete-only higher aggregation, stable hashing, and a minimal CCXT 1m-only adapter capability; focused tests pass 44/44. No production submit, live authority, VPS action, or new qualification is claimed.
