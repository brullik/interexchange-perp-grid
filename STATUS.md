...
- **Current checkpoint:** protected `main` `459a3d6eef5f788066dc73cba2298918abbf9945` continuously enables all seven configured venues for capability-gated public observation while preserving immutable Wave 1 live authority; post-merge run `32462202532` passed all five jobs. Exposure-safe upgrade bootstrap remediation now passes the full local gate: it takes an exact old-image backup before target migration while the legacy process is paused, then arms both the new gate and old-compatible risk-stage/account-flatten controls from the verified target image. Thus first install, failed rollback health, and gate-release failure cannot expose normal entry even when the previous image predates the new CLI/table; owner pause state is restored exactly and emergency reduction remains available. Exact checkpoint commit, CI/artifacts, and final review remain required. Remaining independent software work after this gate is complete owner-onboarding/canary handoff
...
| Exposure-safe immutable upgrade | LOCAL_GATE_PASS / EXACT_GATE_REQUIRED | With the legacy app paused, `shadow-upgrade.sh` first takes a backup through the verified persisted old digest and only then runs the gate/migration through the verified target digest, so rollback never depends on target-mutated state. Arm atomically records the prior owner risk-stage freeze, raises the old-compatible stage freeze, owns the old-compatible account-wide flatten lease, and raises the dedicated new-action gate. The paused old process is killed only at zero active actions; otherwise it resumes with normal entry and FLAT reactivation blocked while existing recovery and standalone emergency reduction remain available. New and legacy images therefore remain fail-closed through new health, rollback health/identity failure, or release failure; successful release restores the exact prior owner freeze and only its own lease. Every pre-deploy backup/restart is scoped to the exact persisted old digest. Twenty concurrent arm/create races, restart persistence, owner-freeze preservation, first-upgrade compatibility, pre-migration backup, active-action refusal, backup failure, failed rollback, real rollback, and CLI wiring are executable tests. Full local gate: lock64, Ruff113, mypy111, pytest 686 passed/11 platform-only skips, doctor shadow/live=false, diff-check PASS. Exact commit, Linux/Docker CI artifacts, and final P0/P1/P2=0 review remain required |
...
2026-08-21 — Freeze new live-action creation in the same durable transaction that owns pair admission before stopping an upgrade — an active or newly racing exposure must keep the old service available for recovery, and automatic rollback must never clear an owner pause or release entry before exact health — live journal, deployment CLI/scripts
