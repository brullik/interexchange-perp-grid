...
- **State:** AGGRESSIVE_SYMBIOSIS_V1 / A6_SOFTWARE_LOCAL_PASS / A7_EXACT_GATE_PENDING / LIVE_DISABLED
...
- **Latest local checkpoint:** the shared LIVE-mode aggressive evaluator emits short-lived, hash-protected, non-authorizing intents which the existing canary runtime revalidates against exact qualification/model/profile/runtime identities before protected IOC journal ownership. Canary is locked to minimum-notional level 1 and cumulative hard risk 1 USDT. A separate exact canary artifact plus separate owner phrase promotes only the local laptop to `pilot_a`; its durable journal projection prevents duplicate levels across restart, permits one route/levels 1–5 under cumulative 5-USDT journal/risk gates, and requires a real 28,800-second service receipt before stage acceptance. Both Windows paths keep credentials in DPAPI/S4U, require local unlock and Telegram challenge, restore `shadow/live=false` on every exit, and leave any nonterminal action with the existing supervisor/recovery owner. Windows-equivalent verification passed lock64, Ruff134, strict mypy72, pytest794 passed/16 platform skips and doctor `shadow/live_orders_allowed=false`. Exact CI and independent review remain pending; no real order, live consent, accepted laptop evidence, or VPS action is claimed.
