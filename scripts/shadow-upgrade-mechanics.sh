...
if [[ -n "$previous_image" && -n "$previous_sha" ]]; then
  ...
  if ! run_previous_compose run --rm --no-deps app interexchange-grid backup-state \
    ...
    elif ! bash "$(dirname "$0")/shadow-deploy-mechanics.sh" "$previous_image" "$previous_sha"; then
...
upgrade_status=0
if bash "$(dirname "$0")/shadow-deploy-mechanics.sh" "$1" "$2"; then
...
if [[ "$upgrade_status" -ne 0 ]]; then
  ...
  upgrade_gate_armed=true
  if ! bash "$(dirname "$0")/shadow-deploy-mechanics.sh" "$previous_image" "$previous_sha"; then
